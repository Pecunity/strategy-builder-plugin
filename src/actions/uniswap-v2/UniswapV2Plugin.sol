...
error UniswapV2Facet__FailedToApproveTokens();
error UniswapV2Facet__PoolPairDoesNotExist();
error UniswapV2Facet__NotZeroAmountForBothTokensAllowed();
error UniswapV2Facet__NoValidPercentageAmount();
error UniswapV2Facet__NoZeroAmountValid();

contract UniswapV2Plugin is BasePlugin {
    ...
    modifier validPercentage(uint256 percentage) {
        if (percentage == 0 || percentage > PERCENTAGE_FACTOR) {
            revert UniswapV2Facet__NoValidPercentageAmount();
        }
        _;
    }
    ...
    modifier nonZeroAmount(uint256 amount) {
        if (amount == 0) {
            revert UniswapV2Facet__NoZeroAmountValid();
        }
        _;
    }
    ...
    // {
    //     uint256[] memory amountsOut =
    //         router.swapExactETHForTokens{value: amountIn}(amountOutMin, path, address(this), _deadline());
    ...
    //     uint256[] memory amountsOut =
    //         router.swapTokensForExactETH(amountOut, amountInMax, path, address(this), _deadline());
    ...
    //     uint256[] memory amountsOut =
    //         router.swapExactTokensForETH(amountIn, amountOutMin, path, address(this), _deadline());
    ...
    //     uint256[] memory amountsOut =
    //         router.swapETHForExactTokens{value: amountInMax}(amountOut, path, address(this), _deadline());
    ...
    //         revert UniswapV2Facet__NotZeroAmountForBothTokensAllowed();
    //     }
    ...
    //     _approveLPTokens(tokenA, tokenB, amountADesired, amountBDesired);

    //     (uint256 amountA, uint256 amountB, uint256 liquidity) = router.addLiquidity(
    //         tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, address(this), _deadline()
    //     );
    ...
    //         router.addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, 0, 0, address(this), _deadline());
    ...
    //         router.removeLiquidity(tokenA, tokenB, liquidity, amountAMin, amountBMin, address(this), _deadline());
    ...
    //         token, amountTokenDesired, amountTokenMin, amountETHMin, address(this), _deadline()
    //     );
    ...
    //         router.removeLiquidityETH(token, liquidity, amountTokenMin, amountETHMin, address(this), _deadline());
    ...
    function _approveToken(address token, uint256 amount) internal {
        ...
        if (!success) {
            revert UniswapV2Facet__FailedToApproveTokens();
        }
    }

    function _approveLPTokens(address tokenA, address tokenB, uint256 amountA, uint256 amountB) internal {
        bool tokenAApproval = IERC20(tokenA).approve(address(router), amountA);
        bool tokenBApproval = IERC20(tokenB).approve(address(router), amountB);

        if (!tokenAApproval || !tokenBApproval) {
            revert UniswapV2Facet__FailedToApproveTokens();
        }
    }
    ...
    function _getPoolPair(address tokenA, address tokenB) internal view returns (address) {
        ...
        if (_poolPair == address(0)) {
            revert UniswapV2Facet__PoolPairDoesNotExist();
        }
        ...
        returns (uint256 amountForLp)
    {
        ...
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(poolPair).getReserves();
    ...
    function _swap(address tokenIn, address tokenOut, uint256 amountIn) internal returns (uint256) {
        ...
        uint256[] memory amountsOut = router.swapExactTokensForTokens(amountIn, 0, path, address(this), _deadline());
    ...
    function _swapETH(address token, uint256 amountIn) internal returns (uint256) {
        ...
        uint256[] memory amountsOut = router.swapExactETHForTokens{value: amountIn}(0, path, address(this), _deadline());
        emit TokenSwap(path, amountsOut);
    ...
    function _swapToETH(address token, uint256 amountIn) internal returns (uint256) {
        ...
        uint256[] memory amountsOut = router.swapExactTokensForETH(amountIn, 0, path, address(this), _deadline());
