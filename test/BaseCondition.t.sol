...
contract BaseConditionTest is Test {
    ...
    modifier conditionActivated(uint16 _id) {
        vm.prank(wallet);
    ...
    function test_checkCondition(uint16 _id) external conditionActivated(_id) {
    ...
    function test_addStrategyToCondition_Success(uint16 _id) external conditionActivated(_id) {
        uint256 strategyNo = 5;
        for (uint256 i = 0; i < 5; i++) {
            vm.prank(wallet);
            condition.addStrategyToCondition(_id, uint16(i));
        }

        uint16[] memory _strategies = condition.strategies(wallet, _id);
