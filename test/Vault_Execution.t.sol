...
contract StrategyVaultExecutionTest is Test {
    ...
    // address public constant vault = 0x1B78A26cA48168338d547030e3609109E7100b4B;
    address public constant STRATEGY_VAULT_FACTORY = 0xc245f625b416F8Da96dD0Bfc3d190cd9FeC262A3;
    ...
    address public constant MATH_ACTION = 0x5718B9bA5Dfa9374e9418C484983A2F2Cb41c8a8;
    address public constant ERC20_TOKEN_BALANCE_CONDITION = 0x9C736C92997F7C9d67c2CcDa6Ba24281498B8c64;
    address public constant PANCAKE_SWAP_V3_POSITION_RANGE_CHECKER = 0xea97E6d13Dc7878b7876516a7a8DCeD5d8f8eBB7;
    ...
    function setUp() public {
        ...
        vm.prank(wallet);
        vault = payable(StrategyVaultFactory(STRATEGY_VAULT_FACTORY).deployVaultDeterministic(salt));
    ...
    function test_activate_automation_vault() external {
        ...
        uint32 strategyId = 3797737133;
        address _vault = 0xbA70B15cFE7082cCc1E5Ad8C0A877e3383C6231B;
    ...
    function checkRangeCondition() internal view {
        uint256 lpPosition =
            abi.decode(IStrategyVault(vault).getContextVariable(CONTEXT_ID, LP_POSITION_KEY), (uint256));
    ...
    function changePriceToUpperAndExecute() internal {
        ...
        vm.prank(EXECUTOR);
        IStrategyVault(vault).executeAutomation(STRATEGY_ID_LOWER_TICK, EXECUTOR);

        uint256 removeLpAmount =
            abi.decode(IStrategyVault(vault).getContextVariable(CONTEXT_ID, REMOVE_LP_AMOUNT_KEY), (uint256));

        uint256 swappedAmount =
            abi.decode(IStrategyVault(vault).getContextVariable(CONTEXT_ID, SWAPPED_AMOUNT_KEY), (uint256));

        uint256 provideLPAmount =
            abi.decode(IStrategyVault(vault).getContextVariable(CONTEXT_ID, PROVIDE_LP_AMOUNT_KEY), (uint256));
        uint256 repayAmount = abi.decode(IStrategyVault(vault).getContextVariable(CONTEXT_ID, REPAY_AMOUNT), (uint256));
        uint256 borrowedAmount =
            abi.decode(IStrategyVault(vault).getContextVariable(CONTEXT_ID, BORROW_AMOUNT_KEY), (uint256));
    ...
    function createThirdStrategy() internal {
        IStrategyVault.StrategyStep[] memory steps = new IStrategyVault.StrategyStep[](3);

        IStrategyVault.Action[] memory firstStepActions = new IStrategyVault.Action[](4);
        IStrategyVault.Action[] memory secondStepActions = new IStrategyVault.Action[](1);
        IStrategyVault.Action[] memory thirdStepActions = new IStrategyVault.Action[](4);
        ...
        firstStepActions[0] = IStrategyVault.Action({
            selector: IPancakeSwapV3LPActions.removeLiquidityPercentage.selector,
            parameter: abi.encode(vault, 0, PERCENTAGE_LP),
            value: 0,
        ...
        firstStepActions[1] = IStrategyVault.Action({
            selector: IStrategyVault.storeConextVariable.selector,
            parameter: abi.encode(CONTEXT_ID, SWAPPED_AMOUNT_KEY, IStrategyVault.ParamType.UINT256, abi.encode(0)),
            value: 0,
            target: vault,
            actionType: IStrategyVault.ActionType.EXTERNAL,
            inputs: new IStrategyVault.ContextKey[](0), // LP position ID
            output: IStrategyVault.ContextKey({ // Empty struct
                key: "",
                parameterReplacement: IStrategyVault.Parameter({
                    offset: 0,
        ...
        firstStepActions[2] = IStrategyVault.Action({
            selector: IStrategyVault.storeConextVariable.selector,
            parameter: abi.encode(CONTEXT_ID, REPAY_AMOUNT, IStrategyVault.ParamType.UINT256, abi.encode(0)),
            value: 0,
            target: vault,
            actionType: IStrategyVault.ActionType.EXTERNAL,
            inputs: new IStrategyVault.ContextKey[](0), // LP position ID
        ...
        firstStepActions[3] = IStrategyVault.Action({
            selector: IStrategyVault.storeConextVariable.selector,
            parameter: abi.encode(CONTEXT_ID, BORROW_AMOUNT_KEY, IStrategyVault.ParamType.UINT256, abi.encode(0)),
            value: 0,
            target: vault,
            actionType: IStrategyVault.ActionType.EXTERNAL,
            inputs: new IStrategyVault.ContextKey[](0), // LP position ID
        ...
        steps[0] = IStrategyVault.StrategyStep({
            condition: IStrategyVault.Condition({conditionAddress: address(0), id: 0, result0: 0, result1: 1}),
            actions: firstStepActions
        });

        // Second Step: swapp all wBNB if some exist

        IStrategyVault.Condition memory coinBalanceCondition = IStrategyVault.Condition({
        ...
        secondStepActions[0] = IStrategyVault.Action({
            selector: IPancakeSwapV3SwapActions.swapInputSinglePercentage.selector,
            parameter: abi.encode(vault, PERCENTAGE_SWAP, wBNB, USDT, POOL_FEE, false),
            value: 0,
        ...
        steps[1] = IStrategyVault.StrategyStep({condition: coinBalanceCondition, actions: secondStepActions});

        // // Third Step: adjust aave position and provide lp again

        thirdStepActions[0] = IStrategyVault.Action({
            selector: IAaveV3Actions.repayPercentageOfBalance.selector,
            parameter: abi.encode(vault, USDT, REAY_PERCENTAGE, 2),
            value: 0,
        ...
        thirdStepActions[1] = IStrategyVault.Action({
            selector: IAaveV3Actions.borrowToHealthFactor.selector,
            parameter: abi.encode(vault, USDT, HEALTH_FACTOR, INTEREST_RATE_MODE),
            value: 0,
            target: AAVE_V3_Actions,
            actionType: IStrategyVault.ActionType.INTERNAL_ACTION,
            inputs: new IStrategyVault.ContextKey[](0), // Empty array
            output: IStrategyVault.ContextKey({ // save the borrowed amount into context
        ...
        thirdStepActions[2] = IStrategyVault.Action({
        ...
        IStrategyVault.ContextKey[] memory lpInputs = new IStrategyVault.ContextKey[](1);
        lpInputs[0] = IStrategyVault.ContextKey({
            key: PROVIDE_LP_AMOUNT_KEY,
            parameterReplacement: IStrategyVault.Parameter({
                offset: 32,
                length: 32,
        ...
        thirdStepActions[3] = IStrategyVault.Action({
        ...
        steps[2] = IStrategyVault.StrategyStep({
            condition: IStrategyVault.Condition({conditionAddress: address(0), id: 0, result0: 0, result1: 0}),
            actions: thirdStepActions
        });

        vm.prank(wallet);

        bytes memory addConditionERC20BalanceData = abi.encodeCall(
            CoinOrERC20BalanceCondition.addCondition,
            (
        ...
        IStrategyVault(vault).execute(ERC20_TOKEN_BALANCE_CONDITION, 0, addConditionERC20BalanceData);
        ...
        vm.prank(wallet);
        IStrategyVault(vault).createStrategyWithExistingContext(STRATEGY_ID_LOWER_TICK, address(0), steps, CONTEXT_ID);

        vm.prank(wallet);

        bytes memory addConditionData = abi.encodeCall(
            IPancakeSwapV3PositionRangeChecker.addCondition,
            (
        ...
        IStrategyVault(vault).execute(PANCAKE_SWAP_V3_POSITION_RANGE_CHECKER, 0, addConditionData);
        // IPancakeSwapV3PositionRangeChecker(PANCAKE_SWAP_V3_POSITION_RANGE_CHECKER).addCondition(
        //     CONDITION_ID_LOWER_TICK,
        //     IPancakeSwapV3PositionRangeChecker.Condition({
        //         contextId: CONTEXT_ID,
    ...
    function createSecondStrategy() internal {
        ...
        IStrategyVault.Action[] memory actions = new IStrategyVault.Action[](3);
        ...
        actions[0] = IStrategyVault.Action({
            ...
            parameter: abi.encode(collectParams),
            value: 0,
            target: PANCAKE_SWAP_V3_LP_ACTIONS,
            actionType: IStrategyVault.ActionType.INTERNAL_ACTION,
            inputs: inputs, // LP position ID
        ...
        actions[1] = IStrategyVault.Action({
            ...
            parameter: abi.encode(vault, wBNB, PERCENTAGE_AAVE),
            value: 0,
            target: AAVE_V3_Actions,
            actionType: IStrategyVault.ActionType.INTERNAL_ACTION,
            inputs: new IStrategyVault.ContextKey[](0), // Supply amount from context
            output: IStrategyVault.ContextKey({ // Empty struct
                key: "",
                parameterReplacement: IStrategyVault.Parameter({
                    offset: 0,
        ...
        actions[2] = IStrategyVault.Action({
            selector: IAaveV3Actions.repayPercentageOfBalance.selector,
            parameter: abi.encode(vault, USDT, PERCENTAGE_AAVE, 2),
            value: 0,
            target: AAVE_V3_Actions,
            actionType: IStrategyVault.ActionType.INTERNAL_ACTION,
            inputs: new IStrategyVault.ContextKey[](0), // Supply amount from context
        ...
        vm.prank(wallet);
        IStrategyVault(vault).createStrategyWithExistingContext(STRATEGY_ID_INTERVAL, address(0), steps, CONTEXT_ID);
        ...
        IStrategyVault(vault).execute(TIME_CONDITION, 0, addConditionData);
    ...
    function mockTimeAndExecute() internal {
        ...
        vm.prank(EXECUTOR);
        IStrategyVault(vault).executeAutomation(STRATEGY_ID_INTERVAL, EXECUTOR);
    ...
    function executeFirstStrategy() internal {
        ...
        actions[0] = IStrategyVault.Action({
            ...
            parameter: abi.encode(vault, AMOUNT),
            value: 0,
            target: AAVE_V3_Actions,
            actionType: IStrategyVault.ActionType.INTERNAL_ACTION,
        ...
        IUniswapV3OneSidedLPActions.AddLiquidityOneSidedRangeParams memory lpParams = IUniswapV3OneSidedLPActions
            .AddLiquidityOneSidedRangeParams({tokenIn: USDT, token0: USDT, token1: wBNB, fee: POOL_FEE, recipient: vault});
        actions[2] = IStrategyVault.Action({
            selector: IUniswapV3OneSidedLPActions.addLiquidityOneSidedPercentageRange.selector,
            parameter: abi.encode(PERCENTAGE, 0, lpParams),
            value: 0,
            target: PANCAKE_SWAP_V3_ONE_SIDED_LP_ACTIONS,
            actionType: IStrategyVault.ActionType.INTERNAL_ACTION,
            inputs: lpInputs, // provide one sidedd with the total borrow amount
        ...
        vm.prank(wallet);
        IStrategyVault(vault).storeConextVariable(
            CONTEXT_ID, SUPPLY_AMOUNT_KEY, IStrategyVault.ParamType.UINT256, abi.encode(AMOUNT)
        );

        vm.prank(wallet);
        IStrategyVault(vault).createStrategyWithExistingContext(STRATEGY_ID, address(0), steps, CONTEXT_ID);

        vm.prank(wallet);
        IStrategyVault(vault).executeStrategy(STRATEGY_ID);
        console2.log("=== Strategy 1 Information ===");
        uint256 aBNBBalance = IERC20(aBNB).balanceOf(vault);
        console2.log("BNB Amount Supplied!");
        console2.log("aBNB Amount:", _toDecimalString(aBNBBalance, 18));

        assertTrue(aBNBBalance > 0);

        uint256 usdtBalance = IERC20(debtUSDT).balanceOf(vault);
        ...
        console2.log("=============================");
        uint256 wBNBAmountAfter = IERC20(wBNB).balanceOf(vault);
        console2.log("BNB Amount after execution!");
        console2.log("wBNB Amount:", _toDecimalString(wBNBAmountAfter, 18));

        uint256 usdtBalanceAfter = IERC20(USDT).balanceOf(vault);
